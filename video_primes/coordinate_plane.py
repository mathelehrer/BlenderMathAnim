...
for (i,zero) in enumerate(zeros):
    if i<10:
        ...
        bpy.context.scene.my_tool.x_loc = 5
...
for ob in bpy.context.scene.objects:
    if fnmatch.fnmatch(ob.name, "Latex*"):
        ...
        ob.data.materials.append(get_coordinate_material(color = (0,0.15,0,1),emission=0.25))
    if fnmatch.fnmatch(ob.name,"XYZ Function*"):
        arrows.append(ob)
        arrow_mat = get_coordinate_material()
        ob.data.materials.append(arrow_mat)
        arrow_materials.append(arrow_mat)
...
for i in range(2,11):
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.001, enter_editmode=False, align='WORLD', location=(i, 0,zetas[i-2]), scale=(1, 1, 1))
    sphere = bpy.context.active_object
    sphere.data.materials.append(get_coordinate_material((1,1,0,1)))
    spheres.append(sphere)

ranges = [[1.01,10],[-1.99,0.99]]
for i in range(2,20,2):
    ranges.append([-(i+2)+0.01,-i-0.01])
...
neg_curves=[]   
for r in ranges:
    curve = create_parametric_curve(zeta_fcn, offset=0.0, min=r[0], max=r[1], use_cubic=True, iterations=int(np.floor((r[1]-r[0])/0.04)))
    curve.data.bevel_depth=2*r_curve
    if zeta_fcn(r[0])[2]>0:
        curve.data.materials.append(get_coordinate_material((1.,0.,0.,1)))
    else:
        curve.data.materials.append(get_coordinate_material((0,1,1,1)))
        neg_curves.append(curve)
    curve.data.bevel_factor_start = 1
    curves.append(curve)
    
curve_one_half1 = create_parametric_curve(zeta_one_half_fcn,offset=0.0,min=0,max=49.9,use_cubic=True,iterations=1000)
curve_one_half1.data.bevel_depth = 2*r_curve
curve_one_half1.data.bevel_factor_start = 0  
curve_one_half1.data.materials.append(get_image_texture_material("zeta_one_half_50_arg.png")) 

curve_one_half2 = create_parametric_curve(zeta_one_half_fcn,offset=0.0,min=-49.9,max=0,use_cubic=True,iterations=1000)
curve_one_half2.data.bevel_depth = 2*r_curve
curve_one_half2.data.bevel_factor_start = 1  
curve_one_half2.data.materials.append(get_image_texture_material("zeta_one_half_50_arg_neg.png")) 
...
curve_one_half1.data.bevel_factor_end = 0
curve_one_half1.data.keyframe_insert(data_path="bevel_factor_end")
for label in labels:
    label.rotation_euler[0]=np.pi/2
...
for i,sphere in enumerate(spheres):
    bpy.context.scene.frame_set(offset+30*i)
    sphere.keyframe_insert("scale")
    bpy.context.scene.frame_set(offset+30*i+30)
    sphere.scale[0]=200
    sphere.scale[1]=200
    sphere.scale[2]=200
    sphere.keyframe_insert("scale")
...
for i,curve in enumerate(curves):
    ...
    if i>2:
        camera.data.lens = 130-80*(i-2)/(len(curves)-2)
        camera.data.keyframe_insert("lens")

    
    offset = offset+int(30/(i+1))    
...
curve_one_half1.data.bevel_factor_end = 0
curve_one_half1.data.keyframe_insert(data_path="bevel_factor_end")
curve_one_half2.data.keyframe_insert(data_path="bevel_factor_end")

offset=offset+120
bpy.context.scene.frame_set(offset)
curve_one_half1.data.bevel_factor_end = 1
curve_one_half1.data.keyframe_insert(data_path="bevel_factor_end")
curve_one_half2.data.bevel_factor_end = 0
curve_one_half2.data.keyframe_insert(data_path="bevel_factor_end")
...
for arrow,label in zip(arrows,labels):
    ...
    offset=offset+5
    bpy.context.scene.frame_set(offset)
    arrow.location.z=arrow.location.z-47
    label.location.z=label.location.z-45
...
offset=offset+30
#extension of the full plane
bpy.context.scene.frame_set(offset)
ttf.offset_factor = 0.175
ttf.keyframe_insert("offset_factor")


#growth of the heights
...
#fade out arrows
arrow_alphas = []
for arrow_material in arrow_materials:
    arrow_alpha = arrow_material.node_tree.nodes["Principled BSDF"].inputs[19]
    arrow_alpha.default_value = 1
    arrow_alpha.keyframe_insert("default_value")
    arrow_alphas.append(arrow_alpha)
    
    
ttf.offset_factor = 0.75
ttf.keyframe_insert("offset_factor")
offset=offset+30
bpy.context.scene.frame_set(offset)
for arrow_alpha in arrow_alphas:
    arrow_alpha.default_value = 0
    arrow_alpha.keyframe_insert("default_value")
