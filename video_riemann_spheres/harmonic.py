...
def harmonic(z):
    sum = 0
    for k in range(1,int(np.abs(z))**2):
        sum+=z/k/(k+z)
    return sum
...
sphere = new_sphere(2,8,harmonic,icosphere = False)
#add shape keys
create_profile_shape_key(sphere,harmonic,log=False)
sphere.data.shape_keys.key_blocks["Profile"].value=0.147
