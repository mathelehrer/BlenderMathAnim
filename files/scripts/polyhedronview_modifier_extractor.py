...
for node in geo_nodes:
    ...
    if hasattr(node, 'align_x'):
        filedata += append(' align_x="' + node.align_x, end='"')
    if hasattr(node, 'align_y'):
        filedata += append(' align_y="' + node.align_y, end='"')
    ...
    if hasattr(node, 'font'):
        print(node.font.name)
        filedata += append(' font="' + node.font.name, end='"')
    ...
    if hasattr(node,'material'):
        filedata+=append(' material="'+get_default_value(node.material),end='"')
    ...
    if hasattr(node, 'pivot_mode'):
        filedata += append(' pivot_mode="' + node.pivot_mode, end='"')
    ...
    if hasattr(node, 'overflow'):
        filedata += append(' overflow="' + node.overflow, end='"')
    ...
    if hasattr(node, 'string'):
        filedata += append(' string="' + str(node.string), end='"')
    if hasattr(node, 'solver'):
        filedata += append(' solver="' + str(node.solver), end='"')
...
path = os.path.join(path,"/home/jmartin/PycharmProjects/BlenderMathAnimWorkspace/BlenderMathAnim/files/xml/PolyhedronViewModifier_nodes.xml")
print(path)
