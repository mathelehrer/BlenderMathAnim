...
COXB4_SIGNATURES = {
    "x3o3o4o": [0, 1, 1, 0],  # 8 (tetra)
    "o3o3o4x": [0, 0, 0, 1],  # 16 (cube)
    "o3x3o4o": [0, 1, 0, 0],  # 24 (octa)
    "o3o3x4o": [0, 0, 1, 0],  # 32  (cubocta,tetra)

    "x3x3o4o": [1, -1, 0, 0],  # 48 (trunc_tetra,octa)
    "x3o3o4x": [0, 0, 1, 1],  # 64 (tetra,prism3, prism4,cube)
    "o3o3x4x": [0, 0, 1, -1],  # 64 (trunc_cube,tetra)
    "o3x3o4x": [1, 0, -1, 1],  # 96 (rhombicubocta,octa,prism3)
    "o3x3x4o": [0, 1, -1, 0],  # 96 (trunc_octa,trunc_tetra)
    "x3o3x4o": [1, 1, -1, 0],  # 96 (prism4, cubocta)

    "x3x3x4o": [1, -1, 1, 0],  # 60 (trunc_octa, prism4,trunc_octa)
    "x3x3o4x": [1, -1, 0, -1],  # 60 (trunc_tetra,prism6, prism4, rhombicubocta)
    "x3o3x4x": [1, 0, 1, -1],  # 60 (cubocta, prism3, prism8,trunc_cube)
    "o3x3x4x": [0, 1, -1, 1],  # 60 (trunc_octa,prism3 trunc_tetra)

    "x3x3x4x": [1, -1, 1, -1]  # 120 (trunc_cubocta,prism6,prism8, trunc_octa)
}
...
class CoxB4:
    ...
    def get_cells_in_conjugacy_classes(self, signature=COXB4_SIGNATURES["x3x3x4x"]):
        """
        sort faces into their conjugacy classes
        the representatives of the classes are the faces that contain the vertex
        with index 0
        >>> group = CoxB4("data")
        >>> cc = group.get_cells_in_conjugacy_classes(COXB4_SIGNATURES["x3x3x4x"])
        >>> print(cc)
        ...
        if isinstance(signature, str):
            signature = COXB4_SIGNATURES[signature]
...
if __name__ == '__main__':
    group = CoxB4()
    signs = COXB4_SIGNATURES["x3x3x4x"]
    print(len(group.get_point_cloud(signs)))
    print(len(group.get_edges(signs)))
    print(group.get_cells_in_conjugacy_classes(signs))
