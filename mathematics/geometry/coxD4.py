# we generate the full coxeter group of type D4
...
class CoxD4:
    def __init__(self,path=None):
        ...
        if not os.path.exists(os.path.join(self.path, "coxD4_elements.dat")):
            self.elements = self.generate_elements()
        else:
            self.elements = self.read_elements("coxD4_elements.dat")
    ...
    def get_point_cloud(self,signature =[1,1,1,1],start = None):
        return self.point_cloud(signature,start)

    def point_cloud(self,signature =[1,1,1,1],start = None):
        filename = "coxD4_points" + str(signature).replace(",", "_") + ".dat"
    ...
    def get_edges(self,seed=[1,1,1,1]):
        filename="coxD4_edges"+str(seed).replace(",","_")+".dat"
        if not os.path.exists(os.path.join(self.path, filename)):
    ...
    def get_cells(self,seed=[1,1,1,1]):
        ...
        cells = {}
        filename = "coxD4_cells" + str(seed).replace(",", "_") + ".dat"
        if os.path.exists(os.path.join(self.path, filename)):
            with open(os.path.join(self.path, filename), "r") as f:
                for line in f:
                    parts = line.split("->")
    ...
    def get_faces(self,seed=[1,1,1,1]):
        faces = set()
        filename = "coxD4_faces" + str(seed).replace(",", "_") + ".dat"
        if os.path.exists(os.path.join(self.path, filename)):
            with open(os.path.join(self.path, filename), "r") as f:
                for line in f:
    ...
    def get_cells_in_conjugacy_classes(self, signature=COXD4_SIGNATURES["x3x3x *b3x"]):
        """
        ...
        if isinstance(signature, str):
            signature = COXD4_SIGNATURES[signature]
