...
from mathematics.geometry.coxB3 import CoxB3
...
def generate_data():
    ...
    t0 = 0
    path = "data"
    group = CoxB3(path=path)

    data = read_signatures(os.path.join(path,"CoxB3_signatures.dat"))
    for signature, start in data:
        ...
        if len(vertices)-len(edges)+len(faces)==2:
            print(signature,":",len(vertices), len(edges))
    ...
    """
    [1,1,1]->truncated cube
    [1,1,0]->rhombicuboctahedron
    [1,1,-1]->truncated cuboctahedron
    [1,0,1]->octahedron
    [1,0,0]->octahedron
    [1,0,-1]->truncated octahedron
    [1,-1,1]->rhombicuboctahedron
    [1,-1,0]->truncated cube
    [0,1,1]->rhombicuboctahedron
    [0,1,0]->cube
    [0,1,-1]->truncated cube
    [0,0,1]->cuboctahedron
    """
