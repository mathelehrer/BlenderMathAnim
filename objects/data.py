from geometry_nodes.geometry_nodes_modifier import DataModifier, DataModifier3D
...
class Data3D(BObject):
    def __init__(self,data=None,coordinate_system=None,name="DataObject",**kwargs):
        ...
        if coordinate_system:
            domains = coordinate_system.domains
            lengths = coordinate_system.lengths
            offset = Vector([lengths[i]/(domains[i][1]-domains[i][0])*(coordinate_system.origin[i]-domains[i][0]) for i in range(len(lengths))])
            coordinate_system.add_data(self)
        if data:
            self.data_modifier = DataModifier3D(x_domain=domains[0], y_domain=domains[1],z_domain=domains[2],
                                                width=lengths[0],depth=lengths[1],height=lengths[2],
                                                offset=offset,
                                                **kwargs)
            self.add_mesh_modifier(type='NODES', node_modifier=self.data_modifier)
