...
class Poly600Cell(BObject):
    """The 4D 600-cell (hexacosichoron) projected into 3D, drawn as a single mesh.

    Geometry is generated by ``mathematics.geometry.cell600`` from two
    quaternions; the resulting mesh has 120 vertices, 720 edges, and 1200
    tetrahedral faces (after 3D projection).
    """

    def __init__(self, **kwargs):
        """Construct the projected 600-cell mesh.

        Args:
            **kwargs: Forwarded to :class:`BObject`. Supported keys:
                * ``name`` (str): Defaults to ``'600Cell'``.
                * ``offset`` (float): Stereographic projection offset
                  (controls how vertices on the W axis are projected
                  into 3D). Defaults to 0.1.
