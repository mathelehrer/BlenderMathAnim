...
class NumberLine2(BObject):
    def __init__(self,name='Numberline',**kwargs):
        """Create a number line driven by a :class:`NumberLineModifier`.

        The whole geometry (axis tube, tic marks, tic labels, arrow tip,
        axis label) is generated by a single geometry-nodes modifier so
        all parameters can be animated later (e.g. log/linear toggling,
        zooming, retictic-ing).

        Args:
            name: Object name shown in Blender. Defaults to ``'Numberline'``.
            **kwargs: Forwarded to :class:`NumberLineModifier` and
                :class:`BObject`. Supported keys:
                * ``length`` (float): Total axis length. Defaults to 1.
                * ``radius`` (float): Axis tube radius. Defaults to 0.05.
                * ``domain`` (list[float]): ``[min, max]`` in coordinate
                  units. Defaults to ``[0, 1]``.
                * ``location`` (list[float]): World location. Defaults to
                  ``[0, 0, 0]``.
                * ``n_tics`` (int): Number of major tics. Defaults to 10.
                * ``origin`` (float): Coordinate value at the world origin.
                  Defaults to 0.
                * ``tic_labels`` (str | list[str]): Either ``'AUTO'`` or
                  an explicit list. Defaults to ``'AUTO'``.
                * ``tic_label_digits`` (int): Decimal digits for auto labels.
                  Defaults to 0.
                * ``tic_label_aligned`` (str): One of ``'left'``,
                  ``'center'``, ``'right'``. Defaults to ``'center'``.
                * ``tic_label_shift`` (Vector): Tic-label offset.
                  Defaults to ``[0, 0, 0]``.
                * ``tic_label_suffix`` (str): String appended to every
                  tic label. Defaults to ``""``.
                * ``label_unit`` (str): Unit suffix for the axis label.
                * ``label_position`` (str): One of ``'left'``,
                  ``'right'``, ``'center'``. Defaults to ``'left'``.
                * ``label_closeness`` (float): Axis label distance multiplier.
                  Defaults to 1.
                * ``tip_length`` (float): Arrow-tip length. Defaults to 0.2.
                * ``auto_smooth`` (bool): Enable smooth shading.
                  Defaults to ``True``.
                * ``axis_label`` (str): LaTeX text for the axis label.
                  Defaults to ``'x'``.
                * ``direction`` (str): Axis orientation. One of:

                  - ``'HORIZONTAL'`` -- along world X.
                  - ``'VERTICAL'``   -- along world Z.
                  - ``'DEEP'``       -- along world Y.
                  - ``'NONE'``       -- keep the modifier default.
...
class NumberLine(BObject):
    ...
    def __init__(self,
                 ...
                 shading=None,
                 **kwargs):
        """Create a number line built from mesh primitives (cylinder + cone + tics + labels).

        Older sibling of :class:`NumberLine2`. Uses real Blender objects
        rather than geometry nodes; one-off, not zoomable at animation time.

        Args:
            length: Axis length in world units.
            radius: Axis cylinder radius.
            domain: ``[min, max]`` coordinate-space range covered by the axis.
            n_tics: Number of major tic marks.
            label: LaTeX axis label.
            tic_labels: Either ``'AUTO'`` (auto-generated numeric labels)
                or an explicit list of LaTeX strings.
            include_zero: If ``True``, draw a tic at the zero coordinate.
            direction: Axis orientation. One of:

                * ``'VERTICAL'``   -- along world Z (default).
                * ``'HORIZONTAL'`` -- along world X.
                * ``'DEEP'``       -- along world Y.
            origin: The coordinate-space value placed at the world
                position given by ``location_of_origin``.
            label_digit: Number of decimal digits for auto tic labels.
            label_unit: Unit string appended to auto tic labels.
            location_of_origin: World position of the axis origin.
            label_position: One of ``'left'``, ``'center'``, ``'right'``;
                controls placement of the axis label relative to the axis.
            label_closeness: Multiplier scaling the axis-to-label distance.
            axis_label_closeness: Same idea for the axis-name (not tic) label.
            tip_length: Override for the arrow-tip cone length. ``None``
                uses the default.
            shading: Optional shading mode applied to axis materials.
            **kwargs: Forwarded to children and :class:`BObject`.
                Supported keys:
                * ``dynamic`` (bool): Build tic labels as :class:`DigitalRange`
                  objects (used by :class:`DynamicNumberLine`).
                * ``range`` (list[float]): When ``dynamic=True``, the set
                  of label values to wire up.
                * ``rotation_euler`` (list[float]): Override the default
                  orientation rotation.
                * Standard BObject kwargs (``color``, ``name``, ...).
        """
...
class DynamicNumberLine(NumberLine):
    """A :class:`NumberLine` whose tic labels are :class:`DigitalRange` objects
    so the label values can morph at animation time."""

    def __init__(self,**kwargs):
        """Create a number line with morphable tic labels.

        Equivalent to ``NumberLine(dynamic=True, ...)``.

        Args:
            **kwargs: Same as :class:`NumberLine`. The ``range`` kwarg
                (list of values per tic) is especially relevant -- it
                defines the set of values each label can morph between.
        """
        self.kwargs = kwargs
