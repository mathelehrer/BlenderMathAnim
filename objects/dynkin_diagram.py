...
class DynkinDiagram(BObject):
    ...
    def __init__(self, dim=3,labels=[], graph = None, **kwargs):
        ...
        if graph is not None:
            ...
            if root.name[1]==1:
                self.rings.append(Circle2(center=root_location,radius=0.5,num_points=20,color="plastic_example",
            ...
            for i,child in enumerate(children):
                ...
                if child.name[1]==1:
                    self.rings.append(Circle2(center=child_location, radius=0.5, num_points=20, color="plastic_example",
                                              thickness=1, rotation_euler=[pi / 2, 0, 0]))
                if child.weight>2:
