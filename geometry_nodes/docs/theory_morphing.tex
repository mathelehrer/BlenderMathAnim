...
\begin{enumerate}[leftmargin=1.6em]
  \item \textbf{Arc-length pairing for curve-like shapes.} Convert both
    ...
    \node{MorphModifier}).
  \item \textbf{Python supermesh pass.} Parameterise both meshes over a
    sphere~\cite{kent1992} or a common base
    domain~\cite{praun2001,kraevoy2004}, overlay the two, and emit a single
    mesh with two position attributes. The node then interpolates two
    attributes of one mesh, which is correct by construction --- and
    \node{MorphNode} needs no change at all.
  \item \textbf{Better trajectories.} Once the supermesh exists, replace the
    straight line by an as-rigid-as-possible path~\cite{alexa2000}; a
