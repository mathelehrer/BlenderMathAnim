...
class Node:
    ...
    def from_attributes(cls, tree, attributes):
        ...
        if type == "INPUT_INT":
            ...
            return InputInteger(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                integer=integer)
        ...
        if type == "INPUT_BOOL":
            ...
            return InputBoolean(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                boolean=boolean)
        ...
        if type == "COLLECTION_INFO":
            ...
            return CollectionInfo(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                  node_height=200, transform_space=transform_space)
        ...
        if type == "INPUT_MATERIAL":
            ...
            return InputMaterial(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                 material=material)
        if type == "MESH_VERTEX_NEIGHBORS":
            return VertexNeighbors(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                   node_height=200)
        ...
        if type == "SUBDIVISION_SURFACE":
            return SubdivisionSurface(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                      node_height=200)
        if type == "SPLIT_EDGES":
        ...
        if type == "REALIZE_INSTANCES":
            return RealizeInstances(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                    node_height=200)
        ...
        if type == "SEPARATE_GEOMETRY":
            ...
            return SeparateGeometry(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                    node_height=200, domain=domain)
        if type == "SORT_ELEMENTS":
            domain = attributes["domain"]
            return SortElements(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                domain=domain)
        if type == "DELETE_GEOMETRY":
            domain = attributes["domain"]
            mode = attributes["mode"]
            return DeleteGeometry(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                  node_height=200, domain=domain, mode=mode)
        ...
        if type == "SCALE_ELEMENTS":
            domain = attributes["domain"]
            return ScaleElements(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                 domain=domain)
        if type == "TRANSFORM_GEOMETRY":
            return TransformGeometry(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                     node_height=200)
        ...
        if type == "OBJECT_INFO":
            transform_space = attributes["transform_space"]
            return ObjectInfo(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=400,
        ...
        if type == "GEOMETRY_TO_INSTANCE":
            return GeometryToInstance(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                      node_height=200)
        if type == "INSTANCE_ON_POINTS":
            return InstanceOnPoints(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                    node_height=200)
        ...
        if type == "ATTRIBUTE_DOMAIN_SIZE":
            ...
            return DomainSize(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                              component=component)
        if type == "ATTRIBUTE_STATISTIC":
            data_type = attributes["data_type"]
            domain = attributes["domain"]
            return AttributeStatistic(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                      node_height=200, data_type=data_type, domain=domain)
        ...
        if type == "SAMPLE_INDEX":
            data_type = attributes["data_type"]
            domain = attributes["domain"]
            return SampleIndex(tree, location=location, name=name, label=label, hide=mute, node_height=200,
                               data_type=data_type, domain=domain)
        if type == "SAMPLE_NEAREST_SURFACE":
            data_type = attributes["data_type"]
            return SampleNearestSurface(tree, location=location, name=name, label=label, hide=mute, node_height=200,
                                        data_type=data_type)
        # points
        if type == "POINTS":
        ...
        if type == "EXTRUDE_MESH":
            mode = attributes["mode"]
            return ExtrudeMesh(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                               mode=mode)
        if type == "FLIP_FACES":
            return FlipFaces(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200)
        if type == "DUAL_MESH":
        ...
        if type == "SHORTEST_EDGE_PATHS":
            return ShortestEdgePaths(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                     node_height=200)
        ...
        if type == "OFFSET_CORNER_IN_FACE":
            return OffsetCornerInFace(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                      node_height=200)
        ...
        if type == "SAMPLE_CURVE":
            data_type = attributes["data_type"]
            mode = attributes["mode"]
            return SampleCurve(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                               data_type=data_type, mode=mode)

        # auxiliaries
        if type == "MATH":
            operation = attributes["operation"]
            return MathNode(tree, location=location, name=name, label=label, hide=hide, operation=operation, mute=mute,
                            node_height=200)
        if type == "VECT_MATH":
            operation = attributes["operation"]
        ...
        if type == "COMPARE":
            operation = attributes["operation"]
            data_type = attributes["data_type"]
            return CompareNode(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                               operation=operation, data_type=data_type)
        if type == "MAP_RANGE":
            data_type = attributes["data_type"]
            interpolation_type = attributes["interpolation_type"]
            return MapRange(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                            data_type=data_type, interpolation_type=interpolation_type)
        if type == "MIX":
            data_type = attributes["data_type"]
            ...
            if cf == 'True':
                ...
                clamp_factor = False
            return MixNode(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                           data_type=data_type, factor_mode=factor_mode, clamp_factor=clamp_factor)
        if type == "BOOLEAN_MATH":
            operation = attributes["operation"]
            return BooleanMath(tree, location=location, name=name, label=label, hide=hide, operation=operation,
                               mute=mute, node_height=200)
        ...
        if type == "ALIGN_ROTATION_TO_VECTOR":
            ...
            return AlignRotationToVector(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                         node_height=200, axis=axis, pivot_axis=pivot_axis)
        if type == "ROTATE_ROTATION":
            rotation_space = attributes["rotation_space"]
            return RotateRotation(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                  node_height=200, rotation_space=rotation_space)
        if type == "INVERT_ROTATION":
            return InvertRotation(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                  node_height=200)
        if type == "AXES_TO_ROTATION":
            primary_axis = attributes["primary_axis"]
            secondary_axis = attributes["secondary_axis"]
            return AxesToRotation(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                  node_height=200, primary_axis=primary_axis, secondary_axis=secondary_axis)
        ...
        if type == "EULER_TO_ROTATION":
            return EulerToRotation(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                   node_height=200)

        # switches
        if type == "SWITCH":
            input_type = attributes["input_type"]
            return Switch(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                          input_type=input_type)
        if type == "INDEX_SWITCH":
            data_type = attributes["data_type"]
            return IndexSwitch(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                               data_type=data_type)
        if type == "MENU_SWITCH":
            data_type = attributes["data_type"]
        ...
        if type == "FIELD_ON_DOMAIN":
            data_type = attributes["data_type"]
            domain = attributes["domain"]
            return EvaluateOnDomain(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                    node_height=200, data_type=data_type, domain=domain)

        if type == "FIELD_AT_INDEX":
            data_type = attributes["data_type"]
            domain = attributes["domain"]
            return EvaluateAtIndex(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                   node_height=200, data_type=data_type, domain=domain)
        ...
        if type == "FOREACH_GEOMETRY_ELEMENT_OUTPUT":
            domain = attributes["domain"]
            return ForEachOutput(tree, location=location, name=name, label=label, hide=hide, mute=mute, node_height=200,
                                 domain=domain)
        ...
        if type == "SIMULATION_INPUT":
            return SimulationInput(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                   node_height=200)
        if type == "SIMULATION_OUTPUT":
            return SimulationOutput(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                    node_height=200)

        # Custom Nodes
        if type == "GROUP":
            if "Position Transform" in name:
                return TransformPositionNode(tree, location=location, name=name, label=label, hide=hide, mute=mute,
                                             height=400)
...
class Frame(Node):
    ...
    def add(self, node):
        """ make Frame parent to given node(s), including zone output nodes for Repeat/Simulation/ForEach zones """
        ...
        for n in nodes:
            ...
            for attr in ("repeat_output", "simulation_output", "foreach_output"):
                if hasattr(n, attr):
                    getattr(n, attr).parent = self.node
                    break
...
class FlipFaces(GreenNode):
    def __init__(self, tree, location=(0, 0), selection=None, **kwargs):
        """

        """
        self.node = tree.nodes.new(type="GeometryNodeFlipFaces")
        super().__init__(tree, location=location, **kwargs)
...
class CornersOfEdge(RedNode):
    def __init__(self, tree, location=(0, 0), edge_index=None, weights=None, sort_index=0, std_out="Corner Index",
                 **kwargs):
...
class SampleCurve(GreenNode):
    def __init__(self, tree, location=(0, 0), mode="FACTOR", data_type="FLOAT", all_curves=True, curves=None,
                 value=None, factor=0, **kwargs):
...
class MeshCircle(GreenNode):
    """
    GeometryNodeMeshCircle: a flat n-gon in the XY plane.

    ``fill_type`` is one of ``'NONE'`` (just the boundary edges),
    ``'NGON'`` (a single n-gon face) or ``'TRIANGLE_FAN'`` (a fan of
    triangles).
    """

    def __init__(self, tree, location=(0, 0),
                 vertices=6, radius=1.0, fill_type='NGON', **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeMeshCircle")
        super().__init__(tree, location=location, **kwargs)
        ...
        if isinstance(vertices, int):
            ...
            self.tree.links.new(vertices, self.node.inputs["Vertices"])
...
class VolumeCube(GreenNode):
    ...
    def __init__(self, tree, location=(0, 0),
                 density=None, background=0.0,
                 min=(-1.0, -1.0, -1.0), max=(1.0, 1.0, 1.0),
                 resolution_x=32, resolution_y=32, resolution_z=32,
                 **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeVolumeCube")
        super().__init__(tree, location=location, **kwargs)
        ...
        if isinstance(background, (int, float)):
            self.node.inputs["Background"].default_value = background
        else:
            tree.links.new(background, self.node.inputs["Background"])

        if isinstance(min, (list, tuple, Vector)):
            self.node.inputs["Min"].default_value = tuple(min)
        else:
            tree.links.new(min, self.node.inputs["Min"])

        if isinstance(max, (list, tuple, Vector)):
            self.node.inputs["Max"].default_value = tuple(max)
        else:
            tree.links.new(max, self.node.inputs["Max"])

        for axis_name, val in (("Resolution X", resolution_x),
                               ("Resolution Y", resolution_y),
                               ("Resolution Z", resolution_z)):
            if isinstance(val, int):
                self.node.inputs[axis_name].default_value = val
            else:
                tree.links.new(val, self.node.inputs[axis_name])


class VolumeToMesh(GreenNode):
    """
    GeometryNodeVolumeToMesh: extracts the iso-surface of a volume at a
    given threshold and returns it as a mesh.
    """
    ...
    def __init__(self, tree, location=(0, 0),
                 ...
                 voxel_amount=64,
                 **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeVolumeToMesh")
        super().__init__(tree, location=location, **kwargs)
        ...
        if "Threshold" in self.node.inputs:
            if isinstance(threshold, (int, float)):
                self.node.inputs["Threshold"].default_value = threshold
            else:
                tree.links.new(threshold, self.node.inputs["Threshold"])

        if "Adaptivity" in self.node.inputs:
            if isinstance(adaptivity, (int, float)):
                self.node.inputs["Adaptivity"].default_value = adaptivity
            else:
                tree.links.new(adaptivity, self.node.inputs["Adaptivity"])

        if "Voxel Size" in self.node.inputs:
            if isinstance(voxel_size, (int, float)):
                self.node.inputs["Voxel Size"].default_value = voxel_size
            else:
                tree.links.new(voxel_size, self.node.inputs["Voxel Size"])

        if "Voxel Amount" in self.node.inputs:
            if isinstance(voxel_amount, (int, float)):
                self.node.inputs["Voxel Amount"].default_value = voxel_amount
            else:
                tree.links.new(voxel_amount, self.node.inputs["Voxel Amount"])
...
class SampleNearestSurface(GreenNode):
    ...
    def __init__(self, tree, location=(0, 0),
                 data_type="FLOAT", mesh=None, value=None,
                 group_id=None, sample_position=None, sample_group_id=None,
                 **kwargs
                 ):

        self.node = tree.nodes.new(type="GeometryNodeSampleNearestSurface")
        super().__init__(tree, location=location, **kwargs)
        ...
        if mesh is not None:
            tree.links.new(mesh, self.node.inputs["Geometry"])
...
class GeometryProximity(GreenNode):
    """
    GeometryNodeProximity: returns the closest position on a target
    geometry (and its distance) to a given sample position.

    ``target_element`` is one of ``'POINTS'``, ``'EDGES'``, ``'FACES'``.
    Outputs:
      * ``Position``  – closest point on the target
      * ``Distance``  – Euclidean distance to that point
      * ``Is Valid``  – boolean, true if a target with the matching
                        Group ID was actually found

    The default ``self.std_out`` is the ``Position`` socket; the distance
    is exposed as ``self.distance``.
    """
    ...
    def __init__(self, tree, location=(0, 0),
                 ...
                 **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeProximity")
        super().__init__(tree, location=location, **kwargs)

        self.node.target_element = target_element
        self.std_out = self.node.outputs["Position"]
        self.distance = self.node.outputs["Distance"]
        self.is_valid = self.node.outputs["Is Valid"]

        if geometry is not None:
            tree.links.new(geometry, self.node.inputs["Geometry"])

        if sample_position is not None:
            if isinstance(sample_position, (list, Vector)):
                self.node.inputs["Sample Position"].default_value = sample_position
            else:
                tree.links.new(sample_position, self.node.inputs["Sample Position"])
        ...
        if sample_group_id is not None:
            if isinstance(sample_group_id, int):
                self.node.inputs["Sample Group ID"].default_value = sample_group_id
            else:
                tree.links.new(sample_group_id, self.node.inputs["Sample Group ID"])
...
class IndexOfNearest(GreenNode):
    ...
    returns the index of the nearest point to ``position`` whose Group ID
    differs from the supplied ``group_id`` (use 0 for "match anything").

    Outputs:
      * ``Index``        – integer index of the nearest match
      * ``Has Neighbor`` – boolean, true if a neighbor with a different
                           group id was actually found

    The default ``self.std_out`` is the Index socket.
    """

    def __init__(self, tree, location=(0, 0),
                 position=None, group_id=None, **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeIndexOfNearest")
        super().__init__(tree, location=location, **kwargs)

        self.std_out = self.node.outputs["Index"]
        self.has_neighbor = self.node.outputs["Has Neighbor"]

        if position is not None:
            if isinstance(position, (list, Vector)):
                self.node.inputs["Position"].default_value = position
            else:
                tree.links.new(position, self.node.inputs["Position"])
...
class VertexNeighbors(RedNode):
    def __init__(self, tree, location=(0, 0), **kwargs):
        self.node = tree.nodes.new(type="GeometryNodeInputMeshVertexNeighbors")
        super().__init__(tree, location=location, **kwargs)
        self.std_out = self.node.outputs["Vertex Count"]
...
class AlignRotationToVector(RedNode):
    def __init__(self, tree, location=(0, 0), pivot_axis="AUTO", axis="Z", rotation=None, factor=1, vector=Vector(),
                 **kwargs):
...
class AxesToRotation(BlueNode):
    def __init__(self, tree, location=(0, 0), primary_axis='X', secondary_axis='Z', primary_direction=None,
                 secondary_direction=None, **kwargs):
...
class RandomValue(BlueNode):
    def __init__(self, tree, data_type="FLOAT_VECTOR", location=(0, 0), min=-1 * Vector([1, 1, 1]),
        ...
        if data_type == "FLOAT_VECTOR":
            ...
            self.std_out = self.node.outputs[2]
        elif data_type == "BOOLEAN":
            self.std_out = self.node.outputs[3]
        else:
            self.std_out = self.node.outputs[1]
...
class CombineMatrix(BlueNode):
    def __init__(self, tree, location=(0, 0), col1=Vector(), col2=Vector(), col3=Vector(), col4=Vector([0, 0, 0, 1]),
                 **kwargs):
        """
            creates a 4x4 matrix, that can be used for transformations.

            Each column may be passed as a list/Vector of 4 values where individual
            entries can either be scalars (default values) or node sockets (links).
        """
        ...
        for col_index, col in enumerate((col1, col2, col3, col4), start=1):
            if isinstance(col, (list, Vector)):
                for i in range(len(col)):
                    socket_label = "Column " + str(col_index) + " Row " + str(i + 1)
                    entry = col[i]
                    if isinstance(entry, (int, float)):
                        self.node.inputs[socket_label].default_value = entry
                    else:
                        tree.links.new(entry, self.node.inputs[socket_label])
...
class MixNode(BlueNode):
    def __init__(self, tree, location=(0, 0), data_type="VECTOR", factor_mode="UNIFORM", clamp_factor=False,
                 factor=None, input_a=None, input_b=None, **kwargs):
...
class TransformPositionNode(NodeGroup):
    def __init__(self, tree, **kwargs):
        ...
        super().__init__(tree,
                         inputs={"Position": "VECTOR", "Location": "VECTOR", "Rotation": "ROTATION", "Scale": "VECTOR",
                                 "Undo Transformation": "BOOLEAN"},
    ...
    def fill_group_with_node(self, tree, **kwargs):
        do_transform = make_function(tree, name="Do Transform",
                                     ...
                                     vectors=["position", "scl", "location", "result"], rotations=["rotation"],
                                     hide=False)
...
class BeveledCubeNode(NodeGroup):
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        iop2 = InstanceOnPoints(tree, scale=self.group_inputs.outputs["Size"], instance=subsurf.geometry_out,
                                hide=False)
...
class CoxeterReflectionNode(NodeGroup):
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        tensor = make_function(tree, name="TensorProduct",
                               functions={
                                   "col1": ["1,n_x,n_x,*,2,progress,*,*,-", "0,n_x,n_y,*,2,progress,*,*,-",
                                            "0,n_x,n_z,*,2,progress,*,*,-"],
                                   "col2": ["0,n_y,n_x,*,2,progress,*,*,-", "1,n_y,n_y,*,2,progress,*,*,-",
                                            "0,n_y,n_z,*,2,progress,*,*,-"],
                                   "col3": ["0,n_z,n_x,*,2,progress,*,*,-", "0,n_z,n_y,*,2,progress,*,*,-",
                                            "1,n_z,n_z,*,2,progress,*,*,-"]
                               }, inputs=["n", "progress"], outputs=["col1", "col2", "col3"], scalars=["progress"],
                               vectors=["n", "col1", "col2", "col3"], hide=True)
        ...
        transform_point = TransformPoint(tree, vector=self.group_inputs.outputs["position"],
                                         transform=combine_matrix.std_out)
...
class UnfoldMeshNode(NodeGroup):
    def __init__(self, tree, progression=0, range=20, root_index=0, scale_elements=0.99, **kwargs):
        ...
        super().__init__(tree, inputs={"Mesh": "GEOMETRY", "Progression": "FLOAT", "Range": "FLOAT", "RootIndex": "INT",
                                       "ScaleElements": "FLOAT"},
...
class CycleNode(NodeGroup):
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        select_mover = SeparateGeometry(tree, domain="INSTANCE", selection=mover_function.outputs["selection"],
                                        hide=True)
        ...
        create_geometry_line(tree,
                             [select_numbers, sort_elements, attr_cycle_index, attr_target_position, foreach_mover,
                              repeat, store_pos, final_join],
...
class TranslateToCenterNode(NodeGroup):
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        create_geometry_line(tree, [transform], out=self.group_outputs.inputs["Geometry"],
                             ins=self.group_inputs.outputs["TargetGeometry"])
...
class SlicerNode(NodeGroup):
    def __init__(self, tree, scale=0.7, thickness=0.02, slicing_geometry=None, **kwargs):
        ...
        super().__init__(tree, inputs={"SlicingGeometry": "GEOMETRY", "Geometry": "GEOMETRY", "Scale": "FLOAT",
                                       "Thickness": "FLOAT"},
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        index = Index(tree)
        stat = AttributeStatistic(tree, geometry=self.group_inputs.outputs["Geometry"], domain="FACE",
                                  data_type="FLOAT",
        ...
        # inside repeat zone
        idx = Index(tree)
        compare = CompareNode(tree, data_type="INT", operation="EQUAL", inputs0=idx.std_out,
                              inputs1=repeat.outputs["Iteration"])
        ...
        create_geometry_line(tree, [repeat], ins=self.group_inputs.outputs["Geometry"],
                             out=self.group_outputs.inputs["SlicedGeometry"])
...
class BevelFaces(NodeGroup):
    ...
    def fill_group_with_node(self, tree, **kwargs):
        ...
        create_geometry_line(tree, [foreachface], ins=self.group_inputs.outputs["Geometry"],
                             out=self.group_outputs.inputs["Geometry"])
...
class PolyhedronViewNode(NodeGroup):
    def __init__(self, tree, edge_material=None, edge_radius=0.05, vertex_material=None,
        ...
        super().__init__(tree, inputs={"Mesh": "GEOMETRY", "Edge Radius": "FLOAT", "Edge Material": "MATERIAL",
                                       "Vertex Radius": "FLOAT", "Vertex Material": "MATERIAL",
...
class ShowNormalsNode(NodeGroup):
    def __init__(self, tree, thickness=0.1, length=1, material=None, **kwargs):
        ...
        super().__init__(tree,
                         inputs={"Mesh": "GEOMETRY", "Thickness": "FLOAT", "Length": "FLOAT", "Material": "MATERIAL"},
                         outputs={"Mesh": "GEOMETRY"}, auto_layout=False, name=self.name, **kwargs)
...
def create_socket(tree, node, node_attributes, attributes):
    ...
    if attributes['type'] == 'CUSTOM':
        ...
        if node_attributes['type'] == 'GROUP_INPUT':
            tree.interface.new_socket(attributes['name'], description='', in_out="INPUT",
                                      socket_type=SOCKET_TYPES[attributes['type']])
            if "default_value" in attributes:
                default_value = get_default_value_for_socket(attributes)
                tree.interface.items_tree.get(attributes['name']).default_value = default_value
            return True
        elif node_attributes['type'] == 'GROUP_OUTPUT':
            tree.interface.new_socket(attributes['name'], description='', in_out="OUTPUT",
                                      socket_type=SOCKET_TYPES[attributes['type']])
            return True
        else:
                ...
                if line.startswith("<NODE"):
                    ...
                    while True:
                        ...
                        if line.startswith("</NODE>"):
                            ...
                            elif len(node.inputs) > input_count and node.inputs[
