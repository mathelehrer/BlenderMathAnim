...
class VolumeCube(GreenNode):
    def __init__(self, tree, location=(0, 0),
        ...
        if density is not None:
            self.tree.links.new(density, self.node.inputs["Density"])

        if isinstance(background, (int, float)):
            self.node.inputs["Background"].default_value = background
        else:
            self.tree.links.new(background, self.node.inputs["Background"])

        if isinstance(min, (list, tuple, Vector)):
            self.node.inputs["Min"].default_value = min
        else:
            self.tree.links.new(min, self.node.inputs["Min"])

        if isinstance(max, (list, tuple, Vector)):
            self.node.inputs["Max"].default_value = max
        else:
            self.tree.links.new(max, self.node.inputs["Max"])

        for axis, val in (("X", resolution_x), ("Y", resolution_y), ("Z", resolution_z)):
            key = "Resolution " + axis
            if isinstance(val, int):
                self.node.inputs[key].default_value = val
            else:
                self.tree.links.new(val, self.node.inputs[key])


class VolumeToMesh(GreenNode):
    def __init__(self, tree, location=(0, 0),
                 volume=None,
                 resolution_mode='Grid',  # 'Grid', 'Amount', 'Size'
                 threshold=0.1,
                 adaptivity=0.0,
                 voxel_size=0.3,
                 voxel_amount=64.0, **kwargs
                 ):
    ...
    while (left_empty or right_empty) and len(stack) > 0:
        ...
        if next_element in OPERATORS:
            ...
            if next_element == "*":
                ...
                new_node_structure.right = new_node_math.inputs[1]
                new_node_structure.out = new_node_math.outputs["Vector"]
            elif next_element == "dot" or next_element == "°":
                new_node_math = tree.nodes.new(type="ShaderNodeVectorMath")
